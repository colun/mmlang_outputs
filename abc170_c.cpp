// https://atcoder.jp/contests/abc170/submissions/15746703
/* src: (11 lines)
...
int main() {
    ...
    p.resize(max((int)p.size(), (int)N)); for(int $1=0; $1<N; ++$1) p[$1] = inputInt();
    D.resize(max((int)D.size(), (int)110)); for(int $1=0; $1<110; ++$1) D[$1] = 0;
    for(int $1=0; $1<N; ++$1) D[p[$1] + 5] = 1;
    for(int i=0, $e=200; i<$e; ++i) {
        if(D[X - i + 5] == 0) {
            print(X - i);
            goto $L1;
        }
