// https://atcoder.jp/contests/abc148/submissions/15574764
/* src:
...
int main() {
    N = inputInt();
    T.resize(max((int)T.size(), (int)N)); for(int $1=0; $1<N; ++$1) T[$1] = 0;
    U = queue<int>();
    V = queue<int>();
    U.emplace(inputInt() - 1);
    V.emplace(inputInt() - 1);
    E.resize(max((int)E.size(), (int)N)); for(int $1=0; $1<N; ++$1) E[$1] = vector<int>();
    for(int $1=0; $1<N - 1; ++$1) if(true || ($1)) {
        {
            a = inputInt() - 1;
            b = inputInt() - 1;
        }
    ...
    while(true) {
        ...
        {
            auto $c1 = move(U);
            while(!($c1).empty()) { auto u = ($c1).front(); ($c1).pop();
                T[u] |= 1;
                if(!(T[u] & 2)) {
                    ++cnt;
                }
                for(int $1=0; $1<(int)E[u].size(); ++$1) {
                    const auto & e = E[u][$1];
                    if(!(T[e] & 1)) {
                        U.emplace(e);
                    }
        ...
        {
            auto $c1 = move(V);
            while(!($c1).empty()) { auto v = ($c1).front(); ($c1).pop();
                T[v] |= 2;
                if(T[v] & 1) {
                    --cnt;
                }
                for(int $1=0; $1<(int)E[v].size(); ++$1) {
                    const auto & e = E[v][$1];
                    if(!(T[e] & 2)) {
                        V.emplace(e);
                    }
