// https://atcoder.jp/contests/abc171/submissions/15744353
/* src: (13 lines)
...
int main() {
    ...
    Q = inputInt();
    BC.resize(max((int)BC.size(), (int)Q)); for(int $1=0; $1<Q; ++$1) {
        const auto & $c1 = inputInt();
        const auto & $c2 = inputInt();
        BC[$1] = tuple<int, int>($c1, $c2);
    }
    S = 0LL;
    for(int $1=0; $1<N; ++$1) S += A[$1];
    cnt = vector<int>(1e5 + 8);
    for(int $1=0; $1<N; ++$1) ++cnt[A[$1]];
