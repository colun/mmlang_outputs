// https://atcoder.jp/contests/abc170/submissions/15765013
/* src: (21 lines)
...
int main() {
    ...
    A.resize(max((int)A.size(), (int)N)); B.resize(max((int)B.size(), (int)N)); for(int $1=0; $1<N; ++$1) {
        const auto & $c1 = inputInt();
        const auto & $c2 = inputInt() - 1;
        tie(A[$1], B[$1]) = tuple<int, int>($c1, $c2);
    }
    CD.resize(max((int)CD.size(), (int)Q)); for(int $1=0; $1<Q; ++$1) {
        const auto & $c1 = inputInt() - 1;
        const auto & $c2 = inputInt() - 1;
        CD[$1] = tuple<int, int>($c1, $c2);
    }
    S.resize(max((int)S.size(), (int)2e5 + 8)); for(int $1=0; $1<2e5 + 8; ++$1) S[$1] = set<tuple<int, int> >();
    for(int $1=0; $1<(int)B.size(); ++$1) S[B[$1]].insert(tuple<int, int>(A[$1], $1));
    R = set<tuple<int, int> >();
    for(int s=0; s<(int)S.size(); ++s) if(!S[s].empty()) {
        R.insert(*S[s].rbegin());
    }
