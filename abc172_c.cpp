// https://atcoder.jp/contests/abc172/submissions/15581296
/* src:
...
int main() {
    ...
    A.resize(max((int)A.size(), (int)N)); for(int $1=0; $1<N; ++$1) A[$1] = inputInt64();
    B.resize(max((int)B.size(), (int)M)); for(int $1=0; $1<M; ++$1) B[$1] = inputInt64();
    for(int $1=1; $1<N; ++$1) A[$1] += A[$1 - 1];
    for(int $1=1; $1<M; ++$1) B[$1] += B[$1 - 1];
    ret = (int)(upper_bound(B.begin(), B.end(), K) - B.begin());
    for(int $1=0; $1<(int)A.size(); ++$1) if(A[$1] <= K) {
