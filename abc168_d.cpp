// https://atcoder.jp/contests/abc168/submissions/15796920
/* src: (19 lines)
...
int main() {
    ...
    AB.resize(max((int)AB.size(), (int)M)); for(int $1=0, $e=(0<=M ? M : (int)AB.size()+M); $1<$e; ++$1) {
        const auto & $c1 = inputInt() - 1;
        const auto & $c2 = inputInt() - 1;
        AB[$1] = tuple<int, int>($c1, $c2);
    }
    AB.resize(max((int)AB.size(), (int)M * 2)); for(int $e0=M * 2, $1=(0<=M ? M : (int)AB.size()+M), $e=(0<=$e0 ? $e0 : (int)AB.size()+$e0); $1<$e; ++$1) {
        const auto & $c1 = get<1>(AB[$1 - M]);
        const auto & $c2 = get<0>(AB[$1 - M]);
        AB[$1] = tuple<int, int>($c1, $c2);
    }
    sort(AB.begin(), AB.end());
    C.resize(max((int)C.size(), (int)N)); for(int $1=0, $e=(0<=N ? N : (int)C.size()+N); $1<$e; ++$1) C[$1] = -1;
    C[0] = 0;
    F = queue<int>();
    F.push(0);
    while(!(F).empty()) { int f = (F).front(); (F).pop();
        for(int $b=(int)(lower_bound(AB.begin(), AB.end(), tuple<int, int>(f, -2147483648)) - AB.begin()), $e0=(int)(upper_bound(AB.begin(), AB.end(), tuple<int, int>(f, 2147483647)) - AB.begin()), $1=(0<=$b ? $b : (int)AB.size()+$b), $e=(0<=$e0 ? $e0 : (int)AB.size()+$e0); $1<$e; ++$1) {
    ...
    print("Yes");
    for(int f=1, $e=N; f<$e; ++f) {
        V.resize(max((int)V.size(), (int)0)); for(int $1=0, $e=(0<=0 ? 0 : (int)V.size()+0); $1<$e; ++$1) V[$1] = tuple<int, int>(0, 0);
        V.clear();
        for(int $b=(int)(lower_bound(AB.begin(), AB.end(), tuple<int, int>(f, -2147483648)) - AB.begin()), $e0=(int)(upper_bound(AB.begin(), AB.end(), tuple<int, int>(f, 2147483647)) - AB.begin()), $1=(0<=$b ? $b : (int)AB.size()+$b), $e=(0<=$e0 ? $e0 : (int)AB.size()+$e0); $1<$e; ++$1) {
            const auto & t = get<1>(AB[$1]);
