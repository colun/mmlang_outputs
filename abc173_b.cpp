// https://atcoder.jp/contests/abc173/submissions/15574767
/* src:
...
int main() {
    N = inputInt();
    S.resize(max((int)S.size(), (int)N)); for(int $1=0; $1<N; ++$1) S[$1] = inputWord();
    W = vector<string>({"AC", "WA", "TLE", "RE"});
    for(int $1=0; $1<(int)W.size(); ++$1) revW[W[$1]] = $1;
    C.resize(max((int)C.size(), (int)W.size())); for(int $1=0; $1<W.size(); ++$1) C[$1] = 0;
    for(int $1=0; $1<(int)S.size(); ++$1) ++C[revW[S[$1]]];
    for(int $1=0; $1<(int)W.size(); ++$1) print(W[$1], "x", C[$1]);
    return 0;
}
