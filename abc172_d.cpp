// https://atcoder.jp/contests/abc172/submissions/15581759
/* src:
...
int main() {
    N = inputInt();
    A.resize(max((int)A.size(), (int)N + 1)); for(int $1=0; $1<N + 1; ++$1) A[$1] = 0;
    for(int i=1; i<N + 1; ++i) {
        for(int $1=i; $1<N + 1; $1+=i) ++A[$1];
    }
    sum = 0LL;
    for(int $1=0; $1<(int)A.size(); ++$1) sum += (long long)(A[$1]) * $1;
    print(sum);
    return 0;
}
