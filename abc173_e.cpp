// https://atcoder.jp/contests/abc173/submissions/15575716
/* src:
...
int main() {
    ...
    ret = 1;
    if(P.empty() && K % 2 == 1) {
        for(int $1=0; $1<K; ++$1) ret = mod_mul(ret, M[$1], 1e9 + 7);
    }
    else if(N == K) {
        for(int $1=0; $1<(int)M.size(); ++$1) ret = mod_mul(ret, M[$1], 1e9 + 7);
        for(int $1=0; $1<(int)P.size(); ++$1) ret = mod_mul(ret, P[$1], 1e9 + 7);
    }
    else {
        if(K % 2 == 1) {
