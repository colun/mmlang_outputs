// https://atcoder.jp/contests/abc170/submissions/15746707
/* src: (18 lines)
...
int main() {
    ...
    D.resize(max((int)D.size(), (int)1e6 + 8)); for(int $1=0; $1<1e6 + 8; ++$1) D[$1] = 0;
    for(int $1=0; $1<N; ++$1) ++D[A[$1]];
    sort(A.begin(), A.end());
    ret = 0;
    for(const int & a : A) {
        --D[a];
        for(int k=1, $e=1001; k<$e; ++k) {
            if(a % k == 0) {
                if(D[k]) {
                    goto $L2;
                }
